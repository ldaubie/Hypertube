...
export default{
    ...
    category : "movie",
  	...
  	search (that, context, category) {
  		that.$http.post(`http://localhost:3000/search/list/${context.data.category}/${context.data.page}`, {genres: context.data.genres, voteAverage: {gte: context.data.imdb.i, lte : context.data.imdb.j}, releaseDate:{gte: context.data.anne.i, lte : context.data.anne.j}, sortBy : context.data.sort}, {
            before(request) {
