...
export default {
	...
	{
		context.$http.put(COMMENT_TORRENT_URL + hash, {message : context.comment}).then(
			(res) => {
	...
	{
		context.$http.get(COMMENT_TORRENT_URL + hash).then(
			(res) => {
	...
	{
		context.$http.get(COMMENT_USER_URL + user_id).then(
			(res) => {
