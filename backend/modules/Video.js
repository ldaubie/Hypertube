...
function streamToMP4(source)
{
    let cmd = ffmpeg(source)
        ...
        .audioChannels(2)
        .videoCodec('libx264')
        .outputOptions(['-frag_duration 5000','-preset fast', '-crf 0','-threads 0'])
...
var Video = function ( TorrentHash, fileId, ext, opts )
{
    ...
    //console.log(originalFile)
                        // Original file was downloaded but converted version not ready
                        /*if (path.extname(originalFile) == ext)
                        {
                            headers[headers.length] = {name:'Accept-Range', value: 'bytes'}
                            headers[headers.length] = {name:'Content-Range', value: 'bytes 0-' + vid.filelength}
                            resolve({'header': headers, 'stream': originalFile})
                        }
                        else */if (ext === 'mp4')
                        {
